...
{
	var tokens = lang.toLowerCase().replaceAll("_", "-").split(/-/, 2);
...
export function formatError(err) 
{
	...
	{
		message = message
			.replaceAll(/{(\w+)}/g, function(m, p1) { return err[p1]; })
...
{
	return text.replaceAll(/["&'/<=>`]/g, function(s) 
	{
...
export function bgPageInvoke(method, args) 
{
	return new Promise(function(fulfill, reject) 
	{
		brapi.runtime.sendMessage({dest: "serviceWorker",
																													method: method,
																													args: args}, function(res) 
		{
